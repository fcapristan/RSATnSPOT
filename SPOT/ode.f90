...
!    Input, integer ( kind = 4 ) NEQN, the number of equations.
!
!    Input/output, double precision Y(NEQN), the current solution.
!
!    Input/output, double precision T, the current value of the independent
!    variable.
!
!    Input, double precision TOUT, the desired value of T on output.
!
...
!
!    Workspace, double precision YY(NEQN), used to hold old solution data.
!
!    Input, double precision WT(NEQN), the error weight vector.
!
!    Workspace, double precision P(NEQN).
!
!    Workspace, double precision YP(NEQN), used to hold values of the
!    solution derivative.
!
!    Workspace, double precision YPOUT(NEQN), used to hold values of the
!    solution derivative.
!
!    Workspace, double precision PHI(NEQN,16), contains divided difference
!    information about the polynomial interpolant to the solution.
!
!    Workspace, double precision ALPHA(12), BETA(12), SIG(13).
!
!    Workspace, double precision V(12), W(12), G(13).
!
...
!
!    Workspace, double precision PSI(12), contains information about
!    the polynomial interpolant to the solution.
!
!    Input/output, double precision X, a "working copy" of T, the current value
!    of the independent variable, which is adjusted as the code attempts
!    to take a step.
!
!    Input/output, double precision H, the current stepsize.
!
!    Input/output, double precision HOLD, the last successful stepsize.
!
!    Input/output, logical START, is TRUE on input for the first step.
!    The program initializes data, and sets START to FALSE.
!
!    Input/output, double precision TOLD, the previous value of T.
!
!    Input/output, double precision DELSGN, the sign (+1 or -1) of
!    TOUT - T.
!
...
!
  ...
  double precision absdel
  double precision abseps
  double precision abserr
  double precision alpha(12)
  double precision beta(12)
  logical crash
  ...
  integer ( kind = 4 ) ns
  double precision p(neqn)
  double precision phi(neqn,16)
  logical phase1
  double precision psi(12)
  double precision releps
  double precision relerr
  double precision sig(13)
  logical start
  logical stiff
  double precision t
  ...
  double precision wt(neqn)
  double precision x
  double precision y(neqn)
  double precision yp(neqn)
  double precision ypout(neqn)
  double precision yy(neqn)
!
...
!  Parameters:
!
!    Input/output, double precision X, the value of the independent variable.
!
!    Input/output, double precision Y(NEQN), the approximate solution at the
...
!    Input, integer ( kind = 4 ) NEQN, the number of equations.
!
!    Input/output, double precision H, the suggested stepsize.
!
!    Input/output, double precision EPS, the local error tolerance.
!
!    Input, double precision WT(NEQN), the vector of error weights.
!
!    Input/output, logical START, is set to TRUE before the first step.
!    The program initializes data, and resets START to FALSE.
!
!    Input/output, double precision HOLD, the step size used on the last
!    successful step.
!
...
!
  ...
  logical crash
  double precision eps
  double precision erk
  double precision erkm1
  double precision erkm2
  double precision erkp1
  double precision err
  external f
  double precision fouru
  double precision g(13)
  double precision, dimension ( 13 ) :: gstr = (/ &
  ...
  double precision, dimension ( 13 ) :: two = (/ &
...
!  Parameters:
!
!    Input, double precision X, the point where the solution has been computed.
!
!    Input, double precision Y(NEQN), the computed solution at X.
!
!    Input, double precision XOUT, the point at which the solution is desired.
!
!    Output, double precision YOUT(NEQN), the solution at XOUT.
!
!    Output, double precision YPOUT(NEQN), the derivative of the solution
!    at XOUT.
!
!    Input, integer ( kind = 4 ) NEQN, the number of equations.
!
...
!    successful step.
!
!    Input, double precision PHI(NEQN,16), contains information about the
!    interpolating polynomial.
!
!    Input, double precision PSI(12), contains information about the
!    interpolating polynomial.
!
  implicit none
  ...
  double precision phi(neqn,16)
  double precision psi(12)
  double precision psijm1
  double precision rho(13)
  double precision term
  double precision w(13)
  double precision x
  double precision xout
  double precision y(neqn)
  double precision yout(neqn)
  double precision ypout(neqn)
