...
subroutine propagate(finalConditions,finalDerivs,initialState,&
     ...
     Tlist,tfailinit,deltatfail,timeList,ntime,Isp,&
     GEOptions,filename,nList,&
     PlanetModel,dtInterval,ndtInterval,&
     thetag,mass_time_alt_opt,thrustOffAngleDeg)
  ...
  ! thrustOffAngleDeg   : a list with 2 elements that specify an small offset to the thrust vector (degrees)
  !### Outputs:
  ...
  double precision , intent(in) :: tfailinit,deltatfail
  ...
  !For Calculations
  !##############################################
  double precision ::  longitude,latitude,tfailfinal
  ...
  double precision , dimension(2) :: thrustOffAngleRad
  ...
  tfailfinal = tfailinit + deltatfail

  thrustOffAngleRad = thrustOffAngleDeg*Pi/180.0
  ...
  do while (.TRUE.) ! (latlonalt(3)<=alt_final)
     ...
     if (mass_time_alt_opt==1) then
        if(mass_time_alt_final>=y(4)) then
           print *,'Exiting malfunction turn because final mass at stage reached'
           exit
        end if
     ...
     if (latlonalt(3)<=0) then
        print *,'Exiting malfunction turn because vehicle impacted the ground'
        exit
     end if

     if (tout>=tfailfinal) then
        exit
     end if
...
!!!! this section to handle offsets in thrust angle
    if((norm(Tvector)>0).AND.(tout.GE.tfailinit))then
