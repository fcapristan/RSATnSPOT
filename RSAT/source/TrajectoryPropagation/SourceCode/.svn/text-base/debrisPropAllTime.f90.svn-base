...
subroutine debrisPropagation(finalConditions,finalStateVector,initialState,DebrisVel,&
     mass,Sref,&
  ...
  double precision , dimension(ndtInterval,6),intent(out) :: finalConditions
  double precision , dimension(ndtInterval,7),intent(out) :: finalStateVector
  ...
  finalConditions(index1,:) = (/latlonalt(3),latlonalt(1),latlonalt(2),tout,VrelMag,flag/) 
  ...
  do while ((latlonalt(3)>=0.0).and.(tout <=5.0*3600)) ! stop when it reaches the ground, or 5 hours and still not on the ground
     ...
     index1 = index1 + 1
     finalConditions(index1,:) = (/latlonalt(3),latlonalt(1),latlonalt(2),tout,VrelMag,flag/) 
     finalStateVector(index1,:) = (/y(1),y(2),y(3),y(4),y(5),y(6),tout*omega + thetag0/)
     if(GEOptions.EQ.1)then 
        open(unit=7,file=filename,status='unknown')
...
contains 
  ...
  function rk45(r,V,t,dtmax,dtmin)
    ...
    double precision ,dimension(3) :: resVvec
    double precision :: resV, residual,tol,delta
