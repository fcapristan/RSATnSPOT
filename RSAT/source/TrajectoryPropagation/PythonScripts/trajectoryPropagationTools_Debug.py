...
def interpolateTrajectories(missionList,tlaunchDesired,dt):
    ...
    for indexEvent in range(nevents):
    	...
    	for indexTraj in range(len(missionList)):
            ...
            xM,yM,zM,VxM,VyM,VzM,ux,uy,uz,time,mc,eta,thetag = nominalTrajFromMission(mission,indexEvent)
...
def generateSingleTrajMalFunc(mission,atmProfile,nProp,ThrustOffsetAngDegUppBound=[0,0],timeMalfunc=0.,deltaTimeMalFunc=5.):
    #import sys
    #sys.path.append('../../../safetyAssessmentTool/DebrisPropagation/SourceCode')
    ...
    mass_time_alt_opt = 1
    stages = getNumberOfStages(missionList[0])
    if malFunctionEventIndex>stages:
        print 'Error in propagateTraj.py generateSingleTrajMalFunc'
        print ' malFunctionEventIndex>Nstages'
        exit(2)
    # iterating through each optimal trajectory
    retTrajs = [[0]*malFunctionEventIndex for i in range(nProp)]
    #exit()
    malFuncStage = False
    for index in range(len(missionList)):
        ...
        for randindex in range(nProp):
            stateVectors = []
            densitylist, ulist,vlist,wlist = AP.generateAtmosProfile(densityMeanList,uMeanList,vMeanList,wMeanList,densitySDList,uSDList,vSDList,wSDList)
            fileNameGE = 'TrajProp'+str(indval) + '.txt'
            indval = indval + 1
            for staging in range(malFunctionEventIndex):
                # setting initial condition
                ...
                finalconditions,derivVals = op.propagate(initialstate,m0,mf,sref,minfcdN,cdN,cloption,minfcl,cl,loverd,atmosoption,altitudelist,densitylist,ulist,vlist,wlist,Farray,timelist,isp,geoptions,filename,planetmodel,dtinterval,ndtinterval,thetag,mass_time_alt_opt,ThrustOffsetAngDeg,ncd=len(minfcd),ncl=len(minfcl),ntime=ntime,nlist=len(altitudelist))
                
                tfTemp = fixPropagateResultsGetTime(finalconditions)
                
                
                if timeMalfunc<=tfTemp:
                    
                    ang1 = np.random.uniform(0.,ThrustOffsetAngDegUppBound[0])
                    ang2 = np.random.uniform(0.,ThrustOffsetAngDegUppBound[1])
                    ThrustOffsetAngDeg = [ang1,ang2]
                    finalconditions,derivVals = malFunc.propagate(initialstate,m0,mf,sref,minfcdN,cdN,cloption,minfcl,cl,loverd,atmosoption,altitudelist,densitylist,ulist,vlist,wlist,Farray,timeMalfunc,deltaTimeMalFunc,timelist,isp,geoptions,filename,planetmodel,dtinterval,ndtinterval,thetag,mass_time_alt_opt,ThrustOffsetAngDeg) 
...
def generateRandomMultiTrajMalFunc(missionList,tLaunchDesired,dt,timeMalfunc,deltatfail,ThrustOffsetAngDegUppBound,atmProfile):
    # this function generates entire trajectories
    import numpy as np
    #calculating ECI coordinates for launch 
    import orbitTools # library for coordinate transformation
    ...
    for indexEvent in range(nEvents):
        ...
        if indexEvent ==0 : #first stage...setting initial conditions for rest of cases
            ...
            initialstate = np.array([r0[0],r0[1],r0[2],Veci0[0],Veci0[1],Veci0[2]])
        m0,Fmax,isp,minfcd,cd,sref = vehicleParamFromMission(mission0,indexEvent)
        TOffset = 1.0 #change to add hot or cold trajectories 
        massf = getFinalMassFromMission(mission0,indexEvent)# m0[-1]        
        Fmax = TOffset*Fmax
        etaMin = getEtaMinFromMission(mission0,indexEvent)
        filename = 'pTraj'+str(indexEvent)+'.txt'
        ...
        if etaMin>=0:
            ...
            if timeMalfunc<=tfTemp[-1]:
                ...
                ang2 = np.random.uniform(0.,ThrustOffsetAngDegUppBound[1])
                ThrustOffsetAngDeg = [ang1,ang2]
                print 'TAng',ThrustOffsetAngDeg,cl,loverd
                geoptions=1
                finalconditions,finalderivs = malFunc.propagate(initialstate,m0,propCond,sref,minfcd,cd,cloption,minfcl,cl,loverd,atmosoption,altitudelist,densitylist,ulist,vlist,wlist,Fmat,timeMalfunc,deltatfail,timelist,isp,geoptions,filename,planetmodel,dtinterval,ndtinterval,thetag,propOption,ThrustOffsetAngDeg)
        ...
        if tc[-1]>=tfail : # look in this event (stage)
            ...
            if fileNameGE!=None:
                ...
                for index2 in range(len(xc)):
                    ...
                    print 'R ',(xc[index2]**2+yc[index2]**2+zc[index2]**2)**.5 - 6371000.8
                print 'ThetaG',thetagVector
                print 'omega',omegaE
...
def generateRandomStateVectorMalFunc(missionList,tLaunchDesired,dt,timeMalfunc,deltatfail,ThrustOffsetAngDegUppBound=[0,0],atmProfile=[],fileNameGE=None):
    from scipy import interpolate
    import numpy as np
    import orbitTools
    import data2GoogleEarth as DG
    retList = generateRandomMultiTrajMalFunc(missionList,tLaunchDesired,dt,timeMalfunc,deltatfail,ThrustOffsetAngDegUppBound,atmProfile)
    mission0 = missionList[0]# getting mission parameters for current mission
    omegaE = getAngVelEarth(mission0)  # planet's angular velocity
    tfail = timeMalfunc
    for index in range(len(retList)):
